...
sub _init_untar($self) {
    ...
    if ($tar) {
        my ( $exit, $out, $err ) = OverPAN::IPC::run3 [ $tar, '--version' ];
...
sub _untar ( $self, $file ) {
    ...
    {
        my $ar = $file =~ /\.bz2$/ ? 'j' : 'z';
        ( $exit, $out, $err ) = OverPAN::IPC::run3 [ $self->{tar}, "${ar}tf", $file ];
        last if $exit != 0;
        my $root = $self->_find_tarroot( split /\r?\n/, $out );
        ( $exit, $out, $err ) = OverPAN::IPC::run3 [ $self->{tar}, "${ar}xf", $file, "-o" ];
...
sub _untar_bad ( $self, $file ) {
    ...
    my ( $exit, $out, $err );
    {
        ...
        ( $exit, $out, $err ) = OverPAN::IPC::run3 [ $ar, "-dc", $file ], $temp->filename;
        last if $exit != 0;

        # XXX /usr/bin/tar: Cannot connect to C: resolve failed
        my @opt = $^O eq 'MSWin32' && $self->{tar_kind} ne "bsd" ? ('--force-local') : ();
        ( $exit, $out, $err ) = OverPAN::IPC::run3 [ $self->{tar}, @opt, "-tf", $temp->filename ];
        last if $exit != 0 || !$out;
        my $root = $self->_find_tarroot( split /\r?\n/, $out );
        ( $exit, $out, $err ) = OverPAN::IPC::run3 [ $self->{tar}, @opt, "-xf", $temp->filename, "-o" ];
        return $root if $exit == 0 and -d $root;
    }
    return if !$wantarray;
    return ( undef, $err || $out );
