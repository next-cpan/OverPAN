...
our $DEFAULTS_REPO = {
    5 => q[https://github.com/next-cpan/OverPAN-p5-patches],
    7 => q[https://github.com/next-cpan/OverPAN-p7-patches],
};
...
sub build ( $class, $overpan ) {
    ...
    if ( my $url_or_path = $overpan->source ) {
        if ( $url_or_path =~ m{^http.*github\.com}i ) {
            return OverPAN::Source::GitHub->new( url => $url_or_path );
        }
        ...
        if ( -d $url_or_path ) {
            return OverPAN::Source::FileSys->new( path => $url_or_path );
        }
        ...
        return OverPAN::Source::GitHub->new( url => $github_url );
    }
